...
void insedg(int nr_of_vertices, int nr_of_edges, GPH *graph) {
    ...
    for (i = 0; i < nr_of_edges; i++) {
        ...
        // Adăugăm muchia în lista de adiacență a fiecărui nod
...
void DFS(GPH *graph, int vertex_nr) {
    graph->visited[vertex_nr - 1] = 1;
    printf("%d->", vertex_nr);

    NODE *temp = graph->adjacency_lists[vertex_nr - 1];
    while (temp != NULL) {
...
void BFS(GPH *graph, int start) {
    ...
    while (!is_empty(queue)) {
        ...
        printf("%d ", current + 1); // Adăugăm 1 pentru a afișa nodurile corect
