...
int main()
{
	...
	scanf("%d%*c", &nr_of_vertices);

	printf("cate muchii are graful? ");
	scanf("%d%*c", &nr_of_edges);

    // e indexat de la 1
	GPH *graph = create_graph(nr_of_vertices+1);

	insedg(nr_of_vertices, nr_of_edges, graph);

	printf("de unde plecam in DFS? ");
	scanf("%d%*c", &starting_vertex);
